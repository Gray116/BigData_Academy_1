...
public class PersonMngGUI extends JFrame implements ActionListener{
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == btnInput) {
			// dao를 통해 이름, 직업 등등 입력
			String jname = txtName.getText().trim();
			String name = comJob.getSelectedItem().toString();
			String korStr = txtKor.getText().trim();
			String engStr = txtEng.getText().trim();
			String matStr = txtMat.getText().trim();
			
			if (jname.equals("") || name.equals("") || korStr.equals("") || engStr.equals("")
					|| matStr.equals("")) {
				txtPool.setText("이름, 직업, 국, 영, 수 모두 입력해야 입력 가능!");
				return;
			}
			...
			int result = dao.insertPerson(new PersonDTO(jname, name, kor, eng, mat));
			
			if(result == PersonDAO.SUCCESSED) {
			...
			if(jname.equals("")) {
				txtPool.setText("직업을 선택 후, 직업 조회 가능");
				return;
			}
