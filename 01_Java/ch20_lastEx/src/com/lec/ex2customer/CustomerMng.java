	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource() == btnJoin) {
			...
			if(searchPhone.equals("")) {
				jta.setText("조회하고자하는 전화번호의 뒷자리 혹은 전체를 입력");
				return;
			}
			
			person = dao.selectTel(searchPhone);
			
			if(person.size() == 1) {
				String tel = person.get(0).getTel();
				String name = person.get(0).getName();
				int point = person.get(0).getPoint();
				
				txtPhone.setText(tel);
				txtPhone.setText(name);
				txtPhone.setText(String.valueOf(point));
			} else if (person.size() > 1) {
				jta.setText("");
				for(CustomerDTO customer : person) {
					jta.append(customer.toString() + "\n");
				}
			} else {
				System.out.println("검색되지 않는 번호");
			}
			...
			if(person.size() == 0) {
				...
				jta.setText("고객번호\t전화번호\t고객이름\t포인트\n");
				for(CustomerDTO d : person) {
